...
- `host_config.lane` 是宿主层并发 lane 配置，内容为 `lane_name: max_concurrent` 键值对；未显式填写的 lane 回退到 Host 默认（`llm_api`）与 Service 业务默认（`write_chapter`、`sec_download`、`cn_download`、`hk_download`）的合并值。`llm_api` 是 Host 自治 lane，用户可在此下调/上调运维抓手；`write_chapter` 控制真实章节 scene 的同时写作数（写作 pipeline 的 ThreadPoolExecutor worker 上限与本值同源，顶层 `write_pipeline` orchestration 本身不占这个 lane）；`sec_download` 控制美股财报下载 direct operation 的跨进程串行度；`cn_download` / `hk_download` 控制 A 股 / 港股主源 PDF 下载段的跨进程串行度，不覆盖后续 Docling 转换。
