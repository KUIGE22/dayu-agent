...
- **claim 谓词**：`claim` 的 CAS 条件是 `state ∈ {PENDING_DELIVERY, FAILED_RETRYABLE}`；两者共享"可再次取出投递"的语义。成功 claim 时由 store 通过 `dayu.host.lease.generate_lease_id()` 生成 fence token 写入记录的 `lease_id`，并随 `ReplyOutboxRecord` 返回；调用方必须把它原样保留供后续 ack/nack。
- **mark_delivered 谓词**：CAS 条件是 `state == DELIVERY_IN_PROGRESS AND lease_id == ?`（双条件 fence token CAS）；若记录已是 `DELIVERED` 且 `lease_id` 与传入一致，幂等返回——这是"同一条回复被同一持有者重复确认"的正常场景。若 state 仍为 IN_PROGRESS 但 lease 不匹配（被 stale cleanup 抢占改写），抛 `LeaseExpiredError`；**若 state 已为 DELIVERED 但 lease 不匹配，同样抛 `LeaseExpiredError`**——absorber 终态不允许把"写入未生效、ownership 已变化"伪装成"本次确认成功"。
- **DELIVERED 拒绝失败转移**：一旦 DELIVERED，`mark_failed` 直接报错；业务层不得反悔。
- **FAILED_TERMINAL 幂等**：重复 mark_failed(retryable=False) **必须 lease 匹配**才幂等返回；lease 不匹配同样抛 `LeaseExpiredError`，与 DELIVERED 同构。
- **delivery_key 幂等**：`submit` 用 `INSERT OR IGNORE` + 同 key 的 payload 一致性检查保证"同一 delivery_key 只入库一次且 payload 未被偷偷改掉"。这是对上游重复提交的幂等防线。**冲突语义为显式失败而非"以首成功 payload 为准"**：同 key 不同 payload 时抛 `dayu.contracts.reply_outbox.ReplyOutboxDeliveryKeyConflictError(delivery_key, existing_payload, attempted_payload)`（`ValueError` 子类，定义在契约层让 UI/Service 不依赖 Host 仓储实现模块）；既有 record 状态保持原样，不会被 submit 失败误改成 `FAILED_TERMINAL`，由正常 cleanup / claim 链路自然推进。调用方（`web/routes/chat.py`、`wechat/daemon.py`）以 `REPLY_OUTBOX_DELIVERY_KEY_CONFLICT` 为 grep tag 记录结构化错误日志，不补投递、不重写 payload，让 metric/告警驱动人工介入。
- **Stale in-progress 兜底**：超过 15 分钟仍停在 `DELIVERY_IN_PROGRESS` 的记录，被 `cleanup_stale_reply_outbox_deliveries` 回退到 `FAILED_RETRYABLE` 并打上 `STALE_IN_PROGRESS_ERROR_MESSAGE`，**同时把 `lease_id` 置 NULL**——`FAILED_RETRYABLE` 不持 lease，旧 holder 用旧 lease 撞 `state='delivery_in_progress' AND lease_id = ?` 双条件 CAS 必然 mismatch，抛 `LeaseExpiredError`，不会污染下一次 claim 的新 holder。**返回值仅包含本次事务实际成功改写（rowcount=1）的记录 ID**，事务前快照里被并发线程已 ack/nack/cleanup 的记录会从返回列表中剔除，避免假阳性日志。
- **lease_id 边界**：`lease_id` 是 record 级 fence token，由 `claim` 路径分配。`DELIVERY_IN_PROGRESS` 与 `DELIVERED` / `FAILED_TERMINAL`（吸收态）下保留 lease：吸收态的幂等返回必须用 lease 等值校验来过滤"旧 holder 写入已被接管的记录"。`PENDING_DELIVERY` / `FAILED_RETRYABLE` 下 `lease_id` 为 NULL——这两个状态都意味着"等待下一次 claim 重新分配 ownership"，旧 holder 持有的旧 lease 必须立刻失效，禁止继续改写记录；`mark_failed(retryable=True)` 与 `cleanup_stale_in_progress_deliveries` 都在落库时把 lease 显式置 NULL。`ReplyOutboxRecord.lease_id` 与 service 层 `ReplyDeliveryView.lease_id` 都对外暴露，供渠道侧 round-trip。
...
- **Lease round-trip**：渠道侧从 `claim_delivery` 拿到的 `ReplyDeliveryView.lease_id` 必须原样回传给 `mark_delivery_delivered(..., lease_id=)` / `mark_delivery_failed(ReplyDeliveryFailureRequest(..., lease_id=))`；任何"丢 lease / 重新生成 lease"都会让 fence token CAS 失败抛 `LeaseExpiredError`。Web 路由 ack/nack body 必须携带 `lease_id`（path 里只放 `delivery_id`，避免双真源）。
...
> **资源释放契约**：`KeyboardInterrupt` 同步打断 `asyncio.run()` 时，executor 内 `_finish_run` 所在 `finally` 没有机会执行。`DefaultHostExecutor` 内置以 `run_id` 为键的资源注册表，异步终态与 SIGINT 同步路径通过 atomic-pop 二选一释放；异步路径将阻塞资源释放操作卸载到线程池，避免阻塞 asyncio 事件循环。
