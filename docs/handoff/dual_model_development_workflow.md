...
Before assigning a task, Codex may run `python -m utils.dual_model_pipeline_check` to verify the workflow files are healthy. Automation and CI can use `python -m utils.validate_handoff_docs --json`, `python -m utils.codex_review_gate --json`, or `python -m utils.dual_model_pipeline_check --json` for machine-readable reports. Plain-text and JSON report boundaries defensively redact secret-shaped values even when a caller supplies an unsanitized result object.
